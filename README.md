# PyTorch for Deep Learning Professional Certificate

This repository contains my work from the **PyTorch for Deep Learning Professional Certificate** offered by DeepLearning.AI. As a BIE/Data Engineer and AI Engineer, this certificate strengthens my deep learning expertise for building production-ready AI systems.

## About the Professional Certificate

The PyTorch for Deep Learning Professional Certificate provides hands-on training in building, optimizing, and deploying deep learning models. The curriculum covers practical skills essential for AI/ML engineering roles, from model development to production deployment.

### Key Learning Areas

- **Neural Network Development**: Building and training models with PyTorch
- **Computer Vision**: Image classification, object detection with TorchVision
- **NLP Applications**: Text processing and language models with Hugging Face
- **Model Optimization**: Pruning, quantization, and performance profiling
- **MLOps & Deployment**: ONNX export, MLflow tracking, production pipelines
- **Advanced Architectures**: Transformers, attention mechanisms, diffusion models

## Repository Structure

```
├── Course-1-PyTorch-Exploration/           # PyTorch Exploration ✅
│   ├── Module 1/                           # Introduction to PyTorch
│   ├── Module 2/                           # Neural Networks Fundamentals
│   ├── Module 3/                           # Deep Learning Techniques
│   └── Module 4/                           # Image Classification Projects
├── Course-2-Techniques-and-Ecosystem/      # Techniques and Ecosystem Tools 🔄
│   └── (In Progress)
├── Course-3-Advanced-Architectures/        # Advanced Architectures and Deployment 📅
│   └── (Upcoming)
└── README.md
```

## Courses

### Course 1: PyTorch Exploration ✅ Completed
Foundation course covering PyTorch fundamentals, neural network construction, and image classification projects. Built models for flower classification, medical imaging, and learned core deep learning concepts.

**Key Skills**: PyTorch tensors, autograd, neural network modules, training loops, data loaders

### Course 2: PyTorch: Techniques and Ecosystem Tools 🔄 In Progress
Advanced techniques for model optimization, hyperparameter tuning, and ecosystem tools for production workflows.

**Key Skills**: Optimizers, learning rate scheduling, Optuna, profiling, transfer learning

### Course 3: PyTorch: Advanced Architectures and Deployment 📅 Upcoming
Modern architectures and deployment strategies for production AI systems.

**Key Skills**: Transformers, generative models, ONNX, MLflow, model compression

## Technologies & Tools

**Deep Learning**
- PyTorch, TorchVision, Hugging Face Transformers

**MLOps & Deployment**
- ONNX, MLflow, Optuna

**Data & Computing**
- Python, NumPy, Pandas, Jupyter

**Model Optimization**
- Pruning, Quantization, Profiling

## Hands-on Projects

- **Medical Imaging**: Pneumonia diagnosis from chest X-rays
- **Image Classification**: Multi-class classification (flowers, animals, insects)
- **Text Classification**: NLP with embeddings and transformers
- **Machine Translation**: Sequence-to-sequence models
- **Model Optimization**: Compression and deployment pipelines

## Professional Background

I'm Kaan Kaya, a BIE/Data Engineer and AI Engineer with a software engineering background. I specialize in building scalable data pipelines and deploying AI/ML solutions in production environments.

## Portfolio

Visit [yasarkaankaya.com](https://yasarkaankaya.com) to see this and other AI/ML projects showcased!

## Course Information

**Program**: [PyTorch for Deep Learning Professional Certificate](https://learn.deeplearning.ai/specializations/pytorch-for-deep-learning-professional-certificate/overview)
**Provider**: DeepLearning.AI
**Format**: 3 Courses, Hands-on Labs & Projects

---

*This repository is for educational purposes. Course materials are property of DeepLearning.AI.*
